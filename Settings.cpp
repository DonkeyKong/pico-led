...
bool Settings::validateAll(int numScenes)
{
...
bool Settings::writeToFlashInternal(int sectorOffset)
{
  ...
  flash_range_program(settingsOffsetBytes(sectorOffset), buffer.data(), buffer.size());
...
SettingsManager::SettingsManager(int numScenes)
{
  ...
  if (!settings.validateAll(numScenes))
  {
